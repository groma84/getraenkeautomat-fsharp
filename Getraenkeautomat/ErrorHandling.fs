...
module ErrorHandling = 
    ...
    type Either<'TSuccess, 'TMessage> = 
        ...
        static member FailWith(messages:'TMessage seq) : Either<'TSuccess, 'TMessage> = Either<'TSuccess, 'TMessage>.Bad(messages |> Seq.toList)

        /// Creates a Failure result with the given message.
        static member FailWith(message:'TMessage) : Either<'TSuccess, 'TMessage> = Either<'TSuccess, 'TMessage>.Bad([message])
        
        /// Creates a Success result with the given value.
        static member Succeed(value:'TSuccess) : Either<'TSuccess, 'TMessage> = Either<'TSuccess, 'TMessage>.Ok(value,[])

        /// Creates a Success result with the given value and the given message.
        static member Succeed(value:'TSuccess,message:'TMessage) : Either<'TSuccess, 'TMessage> = Either<'TSuccess, 'TMessage>.Ok(value,[message])

        /// Creates a Success result with the given value and the given message.
        static member Succeed(value:'TSuccess,messages:'TMessage seq) : Either<'TSuccess, 'TMessage> = Either<'TSuccess, 'TMessage>.Ok(value,messages |> Seq.toList)

        /// Executes the given function on a given success or captures the failure
        static member Try(func: Func<_>) : Either<'TSuccess,exn> =        
            try
                Ok(func.Invoke(),[])
            with
            | exn -> Bad[exn]
    ...
    module Trial =  
        ...
        let inline ok<'TSuccess,'TMessage> (x:'TSuccess) : Either<'TSuccess,'TMessage> = Ok(x, [])

        /// Wraps a value in a Success
        let inline pass<'TSuccess,'TMessage> (x:'TSuccess) : Either<'TSuccess,'TMessage> = Ok(x, [])

        /// Wraps a value in a Success and adds a message
        let inline warn<'TSuccess,'TMessage> (msg:'TMessage) (x:'TSuccess) : Either<'TSuccess,'TMessage> = Ok(x,[msg])

        /// Wraps a message in a Failure
        let inline fail<'TSuccess,'Message> (msg:'Message) : Either<'TSuccess,'Message> = Bad([ msg ])

        /// Executes the given function on a given success or captures the exception in a failure
        let inline Catch f x = Either<_,_>.Try(fun () -> f x)
       ...
       /// Flattens a nested result given the Failure types are equal
        let inline flatten (result : Either<Either<_,_>,_>) =
            result |> bind id
    ...
    module AsyncTrial = 
        ...
        type AsyncTrialBuilder() = 
            ...
            member this.Bind(result : Either<'a, 'c>, binder : 'a -> AsyncResult<'b, 'c>) : AsyncResult<'b, 'c> = 
                this.Bind(result
                          |> Async.singleton
                          |> AR, binder)
